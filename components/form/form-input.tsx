...
const FormInput = ({ field, title, required, errorMsg, ...props }: FormInputProps & InputProps) => {
  ...
  return (
    <FormItem className="flex-1 space-y-1">
      ...
      {errorMsg? <FormMessage className="ml-auto text-xs">{errorMsg}</FormMessage> : <FormMessage className="ml-auto text-xs" />}
      </div>
      <FormControl>
