...
const ProfileUser = ({ user }: { user: User }) => {
  ...
  return (
    <Tabs defaultValue={routerTab === "account-settings" ? "account-settings" : "profile"} orientation="vertical" className="flex md:space-x-5 px-5 py-2 md:p-2">
      {isMobile ? (
        <Sheet open={onOpen} onOpenChange={setOnOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="absolute right-5 top-3 z-10">
              <Menu />
            </Button>
          </SheetTrigger>
          <SheetContent>
            <TabsList className="flex flex-col items-start h-full bg-transparent justify-start space-y-2 sticky top-2">
              ...
              <Button variant={"ghost"} className="w-full justify-start text-rose-500 focus:text-rose-500 hover:text-rose-500" onClick={() => signOut()}>
